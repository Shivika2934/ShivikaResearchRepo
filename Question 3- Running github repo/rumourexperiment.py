...
from transformers import TransfoXLLMHeadModel, XLNetLMHeadModel, DistilBertForMaskedLM

from attention_intervention_model import (
	AttentionOverride, BertAttentionOverride
)
...
class Model():
	...
	def attention_intervention(self,
		...
		def intervention_hook(module, input, outputs, attn_override, attn_override_mask):
			attention_override_module = (AttentionOverride if self.is_gpt2 else BertAttentionOverride)(
	...
	def neuron_intervention_single_experiment(self,
		...
		if self.is_txl or self.is_xlnet:
			pass  # to avoid GPU memory error
		with torch.no_grad():
			'''
			...
			# Define context and rep for neuron_intervention
